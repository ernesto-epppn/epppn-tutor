...
const ui = {
  page: {
    ...
    color: "#0f172a",
    background:
      "radial-gradient(circle at top left, rgba(244,114,182,0.08), transparent 30%), radial-gradient(circle at top right, rgba(99,102,241,0.08), transparent 28%)",
...
export default function Page() {
  ...
  const [showReplyReveal, setShowReplyReveal] = useState(false);
  ...
  const usageLine =
    usage?.is_admin
      ? "🛡️ Mode administrateur — accès illimité."
      : usage?.is_pro
      ? "🟣 Ernesto Pro activé — analyses illimitées."
      : usage
      ? `⚡ Capacité Ernesto : ${usage.remaining} / 10 analyses disponibles${
          usage.trial_ends_at ? ` — essai jusqu’au ${new Date(usage.trial_ends_at).toLocaleDateString()}` : ""
        }`
      : null;

  const usagePercent =
    usage && !usage.is_pro && !usage.is_admin ? Math.max(0, Math.min(100, (usage.remaining / 10) * 100)) : 100;
  ...
  return (
    <main style={ui.page}>
      <style>{`
        ...
        .sectionShell { border-radius: 18px; border: 1px solid #eee; overflow: hidden; background: white; box-shadow: 0 10px 28px rgba(15,23,42,0.04); }
        ...
        .pizzaIcon{
          ...
          background: radial-gradient(circle at 50% 50%,
            rgba(252, 211, 77, 1) 0%,
            rgba(251, 191, 36, 1) 62%,
            rgba(194, 65, 12, 1) 78%,
            rgba(154, 52, 18, 1) 100%
          );
          box-shadow: 0 12px 24px rgba(154, 52, 18, 0.18);
          animation: pizzaPulse 900ms ease-in-out infinite;
          overflow: hidden;
        }
        ...
        .pizzaIcon::after{
          ...
          border-radius: 999px;
          background: radial-gradient(circle at 50% 60%, rgba(148,163,184,0.35), transparent 70%);
          filter: blur(1.2px);
          animation: steamUp 1000ms ease-in-out infinite;
        }

        .replyReveal {
          animation: replyReveal 420ms ease;
        }
        ...
        @keyframes steamUp{
          0%{ opacity:0.15; transform: translateX(-50%) translateY(6px) scale(0.9); }
          55%{ opacity:0.55; }
          100%{ opacity:0; transform: translateX(-50%) translateY(-10px) scale(1.15); }
        }
      `}</style>
          ...
          >
            ...
            <div style={{ marginTop: 8, lineHeight: 1.5 }}>
              {paywall.reason === "quota_reached"
                ? "Tu as utilisé les analyses gratuites disponibles."
                : "Ta période d’essai est terminée."}
            </div>

            <div style={{ marginTop: 10, opacity: 0.88 }}>
              Continue avec des analyses illimitées, des réponses plus approfondies et des protocoles précis inspirés de la méthode EPPPN.
            </div>
      ...
      >
        <div>
          <h1 style={{ fontSize: 30, margin: 0, letterSpacing: -0.4 }}>Ernesto — The Pizza, Explained</h1>
          <div style={{ marginTop: 8, opacity: 0.92 }}>
            ...
            <div style={{ marginTop: 4 }}>
              Diagnostic, actions, protocoles et repères visuels pour la pizza et la panification naturelle.
            </div>
          </div>
        </div>
      ...
      <section style={{ marginTop: 16 }}>
        {chat.length === 0 ? (
          ...
          chat.map((m) => (
            ...
            >
              <div className={`bubble ${m.role} ${m.role === "ernesto" && showReplyReveal && m.id === chat[chat.length - 1]?.id ? "replyReveal" : ""}`}>
      ...
      <div className="composer">
        <div style={{ display: "grid", gap: 10 }}>
          ...
          >
            {loading ? <PizzaLoader ms={loadingMs} done={pizzaDone} /> : "Entrée = envoyer · Shift+Entrée = nouvelle ligne"}
          </button>
        </div>
      </div>
    </main>
  ...
  return (
    <div className={`pizzaLoad ${done ? "done" : ""}`} aria-label="Ernesto prépare ta réponse">
      ...
      <div className="pizzaCenter">
        ...
        <div className="pizzaLabel">{done ? "Réponse prête — Ernesto vous la sert." : "Ernesto prépare ta réponse…"}</div>
      </div>
    </div>
  );
}
