...
const EARLY_DIAGNOSTIC_PAGES = 12;
const OCR_SCRIPT_ID = "ernesto-tesseract-runtime-v4";
...
function usefulText(text, minCompact = 34) {
  const s = textStats(text);
  return s.compact >= minCompact && s.words >= 3 && s.alnum >= Math.max(22, Math.floor(s.compact * 0.24));
}

function nativeNeedsOcr(text) {
  const s = textStats(text);
  return s.compact < 150 || s.words < 12;
}
...
function renderScale(page, targetPixels) {
  ...
  return Math.max(2.2, Math.min(4.5, Math.sqrt(targetPixels / area)));
}

async function renderRawPage(page, targetPixels = 14_000_000) {
...
function cropToJpeg(source, sx, sy, sw, sh) {
  ...
  if (data.length > 1_250_000) data = canvas.toDataURL("image/jpeg", 0.72);
  canvas.width = 1;
  canvas.height = 1;
  return data;
}
...
function makeVisionTiles(source) {
  ...
  // Six overlapping zones (2 columns × 3 rows) created directly from the
  // high-resolution colour rendering. Dense book typography therefore reaches
  // the vision model without first shrinking the whole page.
  ...
  const tiles = [];
  for (const y of ys) {
    for (const x of xs) tiles.push(cropToJpeg(source, x, y, cropW, cropH));
  }
  return tiles;
}
...
export default function AdminLargePdfPanelV4() {
  ...
  async function aiOcr(imageDataUrls, pageNumber) {
    ...
    const response = await fetch("/api/admin/ocr-page", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ image_data_urls: imageDataUrls, page_number: pageNumber, language: ocrLanguage }),
    });
    const result = await response.json().catch(() => ({}));
  ...
  async function indexLargePdf(event) {
    ...
    try {
      ...
      await postKnowledge({ action: "progress", job_id: jobId, pages_total: totalPages, pages_done: 0, stage_label: "Analyse OCR HD" });
      ...
      const acceptPageText = async (pageText, pageNumber) => {
        const cleaned = cleanText(pageText);
        if (!usefulText(cleaned, 24)) return false;
      ...
      for (let pageNumber = 1; pageNumber <= totalPages; pageNumber += 1) {
        ...
        try {
          ...
          if (nativeNeedsOcr(nativeText)) {
            ...
            rawCanvas = await renderRawPage(page, 10_000_000);
            enhancedCanvas = enhancedCopy(rawCanvas);
            const w = await ensureWorker();
            if (w.setParameters) await w.setParameters({ tessedit_pageseg_mode: "11", preserve_interword_spaces: "1", user_defined_dpi: "300" });
            setStage(`OCR local · page ${pageNumber}/${totalPages}`);
            const localResult = await w.recognize(enhancedCanvas);
            const localText = cleanText(localResult?.data?.text || "");
            if (scoreText(localText, Number(localResult?.data?.confidence || 0)) > scoreText(bestText, 0)) bestText = localText;

            if (!usefulText(bestText, 42)) {
              // Re-render in colour at higher resolution specifically for vision.
              rawCanvas.width = 1; rawCanvas.height = 1;
              rawCanvas = await renderRawPage(page, 16_000_000);
              const tiles = makeVisionTiles(rawCanvas);
              setStage(`Secours IA HD · page ${pageNumber}/${totalPages}`);
              try {
                const visionText = await aiOcr(tiles, pageNumber);
                aiCount += 1;
          ...
          await acceptPageText(bestText, pageNumber);
          setPagesDone(pageNumber);

          if (pageNumber === EARLY_DIAGNOSTIC_PAGES && pagesWithText === 0) {
            throw new Error("Le diagnostic HD n’a trouvé aucun texte exploitable sur les premières pages. L’import est arrêté automatiquement : il faut contrôler le rendu du scan plutôt que poursuivre 448 pages à vide.");
          }
          ...
          if (pageNumber % 4 === 0 || pageNumber === totalPages) {
            await postKnowledge({
              ...
              stage_label: `OCR HD · ${pagesWithText} pages texte · ${aiCount} secours IA`,
            });
          }
        } finally {
          if (enhancedCanvas) { enhancedCanvas.width = 1; enhancedCanvas.height = 1; }
          if (rawCanvas) { rawCanvas.width = 1; rawCanvas.height = 1; }
          page.cleanup?.();
        }
      }

      if (textBuffer) {
        const chunks = splitText(textBuffer, bufferPageStart, totalPages, nextChunkIndex);
  ...
  async function clearJob(job) {
    const running = ["failed", "extracting", "embedding", "preparing"].includes(job.status);
    if (!running) return;
    if (!window.confirm(`${job.status === "failed" ? "Nettoyer" : "Abandonner"} l’import « ${job.title} » ?`)) return;
    try {
      await postKnowledge({ action: "cancel", job_id: job.id });
      await loadJobs();
    } catch (err) {
      ...
      <div className="hybridCard">
        <div className="hybridHead">
          <div>
            ...
            <h2>Import OCR HD</h2>
            <p>Texte natif → OCR local → vision IA haute définition. Les pages difficiles sont découpées directement depuis un rendu couleur haute résolution, sans réduire d’abord la page entière.</p>
          </div>
          <div className="badge">OCR HD · diagnostic intégré</div>
        </div>
        ...
        {busy || stage !== "Prêt" ? <div className={`progress ${error ? "failed" : stage.includes("✓") ? "done" : ""}`}>
          ...
          <div className="progressFoot"><small>Si 12 pages consécutives restent illisibles, Ernesto s’arrête automatiquement au lieu de poursuivre à vide.</small>{busy ? <button type="button" onClick={abortCurrent}>Abandonner</button> : null}</div>
        </div> : null}

        {error ? <div className="errorBox">{error}</div> : null}
        <div className="notes"><span>✓ Rendu couleur HD</span><span>✓ 6 zones agrandies</span><span>✓ OCR local</span><span>✓ Vision IA de secours</span><span>✓ Arrêt diagnostic</span></div>
